...
async function handleAdminLogin(req, res) {
  ...
  if (username === "admin" && password === "admin123") {
    sendJson(res, 200, { message: "Admin login successful.", token: "admin-token-night-foodies" });
    return;
  }
...
async function handleAddProduct(req, res) {
  ...
  try {
    ...
    sendJson(res, 201, { message: "Product saved.", product });
  } catch (error) {
...
async function handleApiRequest(req, res, pathname) {
  ...
  if (req.method === "POST" && pathname === "/api/admin/products/delete") {
...
function createServerlessHandler(action, basePath = "/api/auth") {
  const routePath = `${basePath}/${action}`;
